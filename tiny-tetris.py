...
sw={'DOWN':'m_d();ro+=1','LEFT':'if k(ro,co-1,cu):co-=1','RIGHT':'if k(ro,co+1'+#
',cu):co+=1','UP':'if k(ro,co,list(zip(*cu[::-1]))):cu=list(zip(*cu[::-1]))'}   #
def k(ro, co, se):                                                              #
...
def m_d():                                                                      #
 ...
 if (not k(ro+1,co,cu) and ro==0 and sys.exit()) or not k(ro+1,co,cu):          #
  for (r,c) in [(r,c)for c in a(q(cu))for r in a(q(cu))]:                       #
...
try:                                                                            #
 while True:                                                                    #
  ...
  try: cr = (cr+1)%d;ch = w.getkey();exec(sw[ch[4:]]+'\nif cr==0:m_d();ro+=1')  #
